...
It contains different kind of information that we may need to convert it to numerical variables to capture the trend.
...
there are 3 levels of ratings: difficult, easy, and moderate; there are very few hikes rated as difficult and most of them are rated as moderate. 

#### Part c {.unnumbered}

Is there anything you don't like about this barplot? For example: check out the x-axis again.

it does not contain the information of the specfic number of hikes for each level of rating. 
...
hikes from 3700-5700. the center of datapoints is 4500. a outcome between 4000-5000 is typical. left-skewed. there are some outliners. 
...
-   What's the difference between `color` and `fill`? color  fill-the bar color; color-the bar outlines
-   Why does adding `color = "white"` improve the visualization? easier to see when it has white lines
-   What did `binwidth` do? defines the width of each bar
-   Why does the histogram become ineffective if the `binwidth` is too big (e.g. 1000 feet)? the graph has limited space
-   Why does the histogram become ineffective if the `binwidth` is too small (e.g. 5 feet)? it is hard to see it clearly
